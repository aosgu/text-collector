...
> 依据：`docs/_facts.md` 与代码（v1.0.2，2026-08-16）。无数据库/无后端；所有**业务数据**在浏览器端 `chrome.storage.local`；另有一份随扩展包分发的**只读配置文件** `config/nav.json`（v0.8.0，见 §1.4 / §3），以及 v1.0.0 起新增的**待办数据**（`todo_` 前缀，见 §1.5–§1.7 / §3）。
> v1.0.2 仅修复内容页 toast 的视觉样式（宿主不再裁剪 toast 自身阴影），**未改变任何实体字段、storage 键、持久化数据或数据层函数接口**；v1.0.1 仅调整待办工作台的 CSS 尺寸规则与浏览器端输入框测量逻辑，同样未改变数据层。
